...
setwd("F:/Research Work/Survey/Data Analysis/Completed/PHI")
...
homSurveyData <- read.csv("PHI.csv")

write.csv(describe(homSurveyData), "DescriptPHI.csv")

#Make a 'working variable' so that originalsurvey data isn't disturbed
homEFA1 <- homSurveyData  #[,c(3,6,7,8,10)]
...
write.csv(homCor, "PHICor.csv")

#Get KMO
homKMO <- KMO(homCor) #Q44 has low KMO and does not load later
...
#Model specification: ML1 -> taken from EFA; items Q26, Q33 and Q15 were dropped (lower factor loadings + helped boost reliability)
cfamodel <- '
ML1 =~ Q38 + Q42
ML2 =~ Q26 + Q40 + Q47
'
...
#Method 2: HTMT Ratio
phiHTMTRatio <- htmt(cfamodel, data = homSurveyData, sample.cov = NULL, missing = "listwise",ordered = NULL, absolute = TRUE)
phiHTMTRatio[2,1]  #check if HTMT ratio is < 0.85