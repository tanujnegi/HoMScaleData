...
setwd("F:/Research Work/Survey/Data Analysis/Completed/MEL")
...
homSurveyData <- read.csv("MEL.csv")

write.csv(describe(homSurveyData), "DescriptMEL.csv")

#Make a 'working variable' so that originalsurvey data isn't disturbed
homEFA1 <- homSurveyData  #[,-c(1,2)]
...
write.csv(homCor, "MELCor.csv")

#Get KMO
homKMO <- KMO(homCor) #all the values are above 0.56
...
#Model specification: ML1 to ML2 -> taken from EFA
cfamodel <- '
ML1 =~ Q43 + Q48 + Q52 + Q58 + Q62
ML2 =~ Q19 + Q24 + Q54 + Q60
...
diag(lcl) <- c(avsqrt[1],avsqrt[2])
lcl  #Check if the diagonal value is highest in the column

#Method 2: HTMT Ratio
melHTMTRatio <- htmt(cfamodel, data = homSurveyData, sample.cov = NULL, missing = "listwise",
                     ordered = NULL, absolute = TRUE)
melHTMTRatio[2,1]  # less than 0.85 >> good discriminant validity