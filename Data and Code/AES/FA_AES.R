...
setwd("F:/Research Work/Survey/Data Analysis/OBJ1/Completed/AES")
...
homSurveyData <- read.csv("AES.csv")

write.csv(describe(homSurveyData), "DescriptAES.csv")

#Make a 'working variable' so that originalsurvey data isn't disturbed
homEFA1 <- homSurveyData #[,c(3,4,5,6,7)]
...
homCor <- cor(homEFA1)
write.csv(homCor, "AESCor.csv")
#Get KMO
homKMO <- KMO(homCor) #all the values are above 0.53
...
#Method 2: HTMT Ratio
aesHTMTRatio <- htmt(cfamodel, data = homSurveyData, sample.cov = NULL, missing = "listwise",ordered = NULL, absolute = TRUE)
aesHTMTRatio[2,1]  #check if HTMT ratio is < 0.85