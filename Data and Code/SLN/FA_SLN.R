...
setwd("F:/Research Work/Survey/Data Analysis/OBJ1/Completed/SLN")
...
homSurveyData <- read.csv("SLN.csv")

write.csv(describe(homSurveyData), "DescriptSLN.csv")

#Make a 'working variable' so that originalsurvey data isn't disturbed
homEFA1 <- homSurveyData #[,-c(1,2,3,4)]
...
#Method 2: HTMT Ratio
slnHTMTRatio <- htmt(cfamodel, data = homSurveyData, sample.cov = NULL, missing = "listwise",
                     ordered = NULL, absolute = TRUE)
slnHTMTRatio[2,1]  # less than 0.85 >> good discriminant validity